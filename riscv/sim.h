...
private:
  InstSimMemory * memRep; // memory for instSim
  ...
  // I changed this by removing DRAM_BASE
  // and replace it with our own physical memory module
  
  bool addr_is_mem(reg_t addr) {
    if(memRep)
		return memRep->addr_is_memory(addr);
    return addr >= DRAM_BASE && addr < DRAM_BASE + memsz;
  }
  char* addr_to_mem(reg_t addr) { 
  	if(memRep) 
		return memRep->getMemoryAtAddr(addr);
	return mem + addr - DRAM_BASE; 
	}
